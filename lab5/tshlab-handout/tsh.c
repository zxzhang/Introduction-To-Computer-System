 ...
 * 
 * NAME: Zhengxiong Zhang    USERID: zhengxiz    Version4: 7/13/2013
 *
 * Version3: There is a bug in Version2. The child process loses the ability
 *           to access the Controlling Terminal. The child cannot write and 
 *           read in terminal. In this new version, I use tcgetpgrp() and 
 *           getpgrp() to fix it. I give the foreground child process group
 *           the power to read and write in the terminal.
 * Note: ./mycat or ./tsh in ./tsh can test this bug. However, ./sdriver can
 *       never find this bug.
 */
#include <assert.h>
#include <stdio.h>
...
/* zxZhang defined */
/* File states */
#define DEF_FLAG    O_CREAT|O_TRUNC|O_RDWR    /* the flags of files in I/O R */
#define DEF_MODE    S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH|S_IWOTH  /* mode */
#define IN          0   /* infile is NULL, getfd() func returns STDIN_FILENO */
#define OUT         1 /* outfile is NULL, getfd() func returns STDOUT_FILENO */
...
/* zxZhang defined */
/* Functions */
int builtinCommand(struct cmdline_tokens tok);/* handle the built in command */
void dobg(char **argv);                           /* handle the "bg" command */
void dofg(char **argv);                           /* handle the "fg" command */
void dokill(char **argv);                       /* handle the "kill" command */
pid_t getargvpid(char **argv); /* get pid from the argv string (PID or %JID) */
int getfd(int option, char *filename);/* get the infile or outfile fd number */
...
/* zxZhang defined */
/*
 * This helper function can handle the "kill" command
 * The writeup has something about "kill". However, ./sdriver does not test for
 * it. I write it. That is OK. I just write it for fun.  :>
 */
void dokill(char **argv)
{
    pid_t pid;
    int jid;
    pid = getargvpid(argv);
...
/* zxZhang defined */
/*
 * This function gets the pid from the argv[1]
 * No matter you put in the pid or %jid, this function will convert it into
 * pid and return it to other functions.
 */
pid_t getargvpid(char **argv)
{
    pid_t pid;
    if ('%' == argv[1][0]) {
        pid = (getjobjid(job_list, atoi(argv[1] + 1))->pid);
    } else {
        pid = atoi(argv[1]);
    }
    return pid;
...
/* zxZhang defined */
/*
 * This function will return the file description number
 * 1. If the file is not NULL, then open it and return the fd.
 * 2. If the file is NULL.
 *    If the option is IN, then returns STDIN_FILENO.
 *    If the option is OUT, then returns TDOUT_FILENO.
 * This function is used for I/O Redirection.
 */
int getfd(int option, char *filename)
{
    int fd;
    if (filename) {
        fd = Open(filename, DEF_FLAG, DEF_MODE);
        return fd;
    } else {
        if (IN == option) {
            return STDIN_FILENO;
...
sigchld_handler(int sig) 
{
    ...
    while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0) {
        jid = pid2jid(pid);
/*
 * If the child process is terminated or stopped, print the information.
 */
...
/*
 * If the child is stopped, set the state to ST. Otherwise, delete the job.
 */
...
/*
 * After I add the WNOHANG to option, waitpid will return 0. So, must add
 * (pid < 0) here to avoid error. Otherwise, it will cause error: SUCCESS.
 */
...
sigint_handler(int sig) 
{
    ...
    pid = fgpid(job_list);
/*
 * If pid is not 0, sent the SIGINT signal.
 */
    if (pid) {
        Kill(-pid, SIGINT);
    }
...
sigtstp_handler(int sig) 
{
    ...
    pid = fgpid(job_list);
/*
 * If pid is not 0, sent the SIGSTP signal.
 */
    if (pid) {
        Kill(-pid, SIGTSTP);
    }
...
/* zxZhang defined */
/* My wrapper functions, I stole them from the csapp.c */
pid_t Fork(void) 
{
    pid_t pid;
